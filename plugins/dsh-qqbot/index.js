 ...
 *
 * 【关键】只 export Config 不会让配置出现在设置页面。
 * Config 走的是 cordis.yml 的加载期配置；设置页面读的是 settings 服务的
 * namespace 注册表。必须调 installSettingsSection 把 namespace 注册进去，
 * 设置页才会渲染出这一节。两者是不同的东西，缺一个就"插件加载了但页面空白"。
 */
 ...
 * 填写可持久化」这条链路是否通畅。
 *
 * installSettingsSection 把 cordis 入口配置作为 base 层注册进 settings 服务：
 * settings 服务在线时用户可在页面上覆盖，服务缺失时自动回落到入口配置，
 * 插件照常工作。
 */
